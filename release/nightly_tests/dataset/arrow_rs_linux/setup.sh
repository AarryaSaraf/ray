...
# --- locate the repo (this script lives at <repo>/release/nightly_tests/dataset/arrow_rs_linux) ---
...
Next, from $SCRIPT_DIR:
  ...
  ./run_all.sh                 # all three experiments, ~40-60 min
  # or one at a time:
  ./exp1_iter_batches.sh       # release test verbatim, needs AWS creds
  ./exp2_fat_col.sh            # local moto S3, no creds needed
  ./exp3_write_parquet.sh      # needs AWS creds, writes locally

Results land in \$OUT_DIR (default: $SCRIPT_DIR/out).
EOF
