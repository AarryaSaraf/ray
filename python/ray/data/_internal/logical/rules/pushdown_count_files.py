...
class PushdownCountFiles(Rule):
    ...
    The scanner must return every row of every file (``preserves_all_rows``),
    and the listing must emit each file exactly once
    (``as_whole_file_indexer``). Both hooks default to "no": a wrong count is
    silent, while declining just falls back to a real read.
    ...
    def apply(self, plan: LogicalPlan) -> LogicalPlan:  # pyrefly: ignore[bad-override]
        ...
        # A predicate, limit or partition pruning would make the metadata
        # ``num_rows`` an overcount -- it is the pre-filter total. Column
        # projection is fine: it changes width, not row count.
        scanner = read_files.scanner
        if not scanner.preserves_all_rows():
            return plan
