...
# for 4" from "4 because that is the fallback". Read once at import, matching
# ``_DEFAULT_NUM_THREADS`` itself: both are module-level, so mutating the
# environment after import has no effect on either and the two cannot disagree.
...
class ArrowRsParquetFileReader(ParquetFileReader):
    ...
    def _num_fragment_read_threads(self) -> int:
        ...
        Ray decodes ``RAY_DATA_READ_FILES_NUM_THREADS`` fragments concurrently per
        read task (default 4) to overlap remote-filesystem latency. That is the
        right default for the PyArrow reader, whose per-fragment decode gives a
        ...
        if _READ_FILES_NUM_THREADS_IS_EXPLICIT:
            return super()._num_fragment_read_threads()
        return 1
