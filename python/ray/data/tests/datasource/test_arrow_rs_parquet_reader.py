...
def test_arrow_rs_defaults_one_fragment_thread(monkeypatch):
    ...
    Ray decodes ``RAY_DATA_READ_FILES_NUM_THREADS`` fragments concurrently per read
    task (default 4) to hide remote-filesystem latency. The arrow-rs path already
    overlaps fetch with decode inside one fragment, so the pool buys no throughput
    and multiplies the working set: measured 2026-08-07, threads=1 saved 96 MiB per
    task on local disk and 44 MiB on S3 at 0.95-1.02x wall
    ...
    assert rs_reader._num_fragment_read_threads() == 1
    ...
    assert pa_reader._num_fragment_read_threads() > 1
...
def test_explicit_num_threads_env_overrides_arrow_rs_default(monkeypatch):
    ...
    Both the env read and the "was it explicit?" flag happen at import time (they
    have to: ``_DEFAULT_NUM_THREADS`` is a module-level ``env_integer``, so the two
    would otherwise be able to disagree). Hence patching both module attributes
    ...
    from ray.data._internal.datasource_v2.readers import (
        ...
        file_reader as fr_mod,
    )

    monkeypatch.setattr(rs_mod, "_READ_FILES_NUM_THREADS_IS_EXPLICIT", True)
    monkeypatch.setattr(fr_mod, "_DEFAULT_NUM_THREADS", 4)

    reader = rs_mod.ArrowRsParquetFileReader(
        filesystem=LocalFileSystem(), target_block_size=128 * 1024 * 1024
    )
    assert reader._num_fragment_read_threads() == 4
